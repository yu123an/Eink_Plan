...
#include <Adafruit_PCF8574.h>   //IO拓展
...
//bmp图像转换
void convert_bmp(int addr)
{
  String bmp_msg = read_eeprom(addr,1024);
  int _long = bmp_msg.length();
  for (int i = 0; i < _long / 2; i++)
  {
    int a, b;
    if (bmp_msg[i * 2] < 58)
    {
    ...
    {
      b = bmp_msg[i * 2 + 1] - 'A' + 10;
    }
    weahter_bmp[i] = a * 16 + b;
    // Serial.print(a[i]);
    // Serial.print(":");
    // Serial.println(b[i]);
  }
...
void draw_HomePage() {
  ...
  do {
    ...
    for (int i = 0; i < totle; i++) {
      ...
      display.drawInvertedBitmap(35,85, _deer,85, 152, INK);
    }
    // 绘制天气事件
    ...
    ug.print("日落：" + sunset);
    display.drawLine(230, 255, 230, 295, INK);
    ...
    display.drawInvertedBitmap(235,253, aperture_logo,160, 42, INK);
    ...
    // display.drawInvertedBitmap(353, 253, moon[moonphase], 42, 42, INK);
    // 绘制小时天气
    //
    ...
    convert_bmp(700);
    display.drawInvertedBitmap(166, 93, weahter_bmp, 64,64, INK);
    display.drawRect(166, 93,64,64, INK);
    convert_bmp(1800);
    display.drawInvertedBitmap(246, 93, weahter_bmp, 64,64, INK);
    display.drawRect(246, 93,64,64, INK);
    convert_bmp(2900);
    display.drawInvertedBitmap(326, 93, weahter_bmp, 64,64, INK);
    display.drawRect(326, 93,64,64, INK);
    int l = random(400);
    int ll = random(400);
    int lll = random(400);
    display.drawInvertedBitmap(166, 170, portal[l % 19], 64,64, INK);
    display.drawInvertedBitmap(246, 170, portal[ll % 19], 64,64, INK);
    display.drawInvertedBitmap(326, 170, portal[lll % 19], 64,64, INK);
    //
...
void setup() {
  ...
  pcf.digitalWrite(4, 0);
  pcf.digitalWrite(7, 1);
  ...
  if ((RST_reason == 0) || (now.Minute() == 2)) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      // }
      if(now.Hour() % 6 == 1){
      get_net(web_weather_icon1);
      write_eeprom(700,JsonMsg);
      get_net(web_weather_icon2);
      write_eeprom(1800,JsonMsg);
      get_net(web_weather_icon3);
      write_eeprom(2900,JsonMsg);
      }
    }
  } else {
    // 其他复位源
  ...
  display.powerOff();
  pcf.digitalWrite(2, 1);
  pcf.digitalWrite(3, 1);
  pcf.digitalWrite(4, 1);
